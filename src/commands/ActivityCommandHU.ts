...
export class ActivityHUCommand implements SlashCommandInterface {

    info: CommandInfo = {
        name: "jatek",
        description: "játék indítása",
        example: "jatek Nézzül együtt",
        usage: "jatek [<activityname>]"
    };
    ...
    async getInvite(userChannel: GuildChannel, option: CommandInteractionOption<CacheType> | null, interaction: CommandInteraction) {
        ...
        try {
            res = await rest.post(`/channels/${userChannel.id}/invites`, { body: { max_age: 86400, max_uses: 0, target_application_id: option?.value, target_type: 2, temporary: false, validate: null, } })
        } catch (err) {
            //@ts-expect-error
